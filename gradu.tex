...
\end{align}
on Kalmanin optimaalinen painomatriisi. $\mathbf{HBH^T}$ ilmaisee mallin taustan havaintoavaruuteen muunnettun kovarianssin ja $\mathbf{BH^T}$ muuttaa havaintojen ja mallin ennusteiden eron mallitilan korjaukseksi. Lisäksi Kalman-suotimessa mallitilan aikakehityksessä tarvittavan ennustusmallin lisäksi on oltava malli, joka kehittää analyysin kovarianssia $\mathbf{A}^n\mathbf{ = (I-KH)B}^{n}$ ajassa eteenpäin (lähde):
...
Kalman-suodin ei kuitenkaan sovellu sellaisenaan käytettäväksi nykyaikaisissa säämalleissa, sillä taustan ja analyysin kovarianssimatriisit muodostuisivat niin suuriksi, ettei niitä nykyisillä tietokoneilla voisi käsitellä (lähde). Lisäksi tangentti-lineaaristen havainto-operaattorien ja mallin kehitystyö vaatii paljon aikaa, ennen kuin data-assimilaatio-järjestelmää voidaan hyödyntää (lähde). Yksinkertaisemmalle järjestelmälle 


\subsection{EnKF IL-mallissa}
